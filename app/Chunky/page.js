...
const page = () => {
    ...
    return (
    <div className='grid grid-cols-1  h-[1180px] mt-20 p-5 xl:grid-cols-3 xl:gap-x-7 '>
        ...
        <div className='flex flex-col gap-y-4'>
          {images.map((image, index) => (
            <button key={index} className='w-[160px] h-[160px] bg-black rounded-[25px]' onClick={() => setSelectedImage(image)}>
              <img className='rounded-[25px] w-[160px] h-[160px] object-cover' src={image} alt={`image-${index}`} />
            </button>
          ))}
        </div>
        <div>
